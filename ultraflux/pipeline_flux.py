...
from .lora_pipeline import FluxLoraLoaderMixin
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            try:
                self.vae.to(device)
                decoded = self.vae.decode(latents.to(self.vae.device,self.vae.dtype), **decode_kwargs)
            except Exception as e:
                logger.warning(f"Failed to decode with partitioned=True; falling back to standard decode. Error: {e}")
                self.vae.to("cpu")
                decoded = self.vae.decode(latents.to(self.vae.device,self.vae.dtype),**decode_kwargs)
